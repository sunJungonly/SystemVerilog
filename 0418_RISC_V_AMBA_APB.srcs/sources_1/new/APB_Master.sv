...
module APB_Master (
    ...
    output logic PSEL2,
    output logic PSEL3,
    input logic [31:0] PRDATA0,  //페리페럴 갯수에 만큼 생김 
    input logic [31:0] PRDATA1,
    input logic [31:0] PRDATA2,
    input logic [31:0] PRDATA3,
    input logic PREADY0,  //페리페럴 갯수에 만큼 생김 
    input logic PREADY1,
    input logic PREADY2,
    input logic PREADY3,
    // Internal Interface Signals
    input  logic transfer,       //실제 데이터를 보내는 최초 신호 trigger signal
    output logic ready,
    ...
    always_comb begin
        ...
        case (state)
            IDLE: begin
                ...
                if (transfer) begin
                    state_next = SETUP;
                    temp_addr_next = addr;  //latching 임시저장소에 저장한다(고정시킨다)
    ...
    APB_Decoder U_APB_Decoder (  //조합회로
        .en (decoder_en),
        .sel(temp_addr_reg),
        .y  (pselx)
    );
