...
//     task display(string name);
//         $display(
//             "[%s] PADDR = %h, PWDATA=%h, PWRITE=%h, PENABLE=%h, PSEL=%h, PRDATA=%h, PREADY=%h, rd_en=%h, fsr=%h, frd=%h",
//             name, PADDR, PWDATA, PWRITE, PENABLE, PSEL, PRDATA, PREADY, rd_en,
//             fsr, frd);
//     endtask  //
// endclass  //transaction
...
//             Mon2SCB_mbox.get(rx_tr);
//             rx_tr.display("SCB");
//             if (rx_tr.PWRITE) begin  // write mode
//                 refRxReg[rx_tr.PADDR[3:2]] = rx_tr.PWDATA;
                
//                 if (refFrd[refRxReg[1]] == rx_tr.frd)  //PASS
//                     $display(
//                         "Rdata PASS!, %h, %h",
//                         refFrd[refRxReg[1]],
//                         rx_tr.frd[7:0]
...
//                 if (refRxReg[0][0] == refFsr[0]) begin  //empty != 0
//                     if (0 == rx_tr.fsr) $display("Not EMPTY!, %h, %h", refRxReg[0][0], refFsr[0]);
//                     else $display("EMPTY!, %h, %h", refRxReg[0][0], refFsr[0]);
//                 end else if(refRxReg[0][1] == refFsr[1]) begin  //full != 0
//                     if (0 == rx_tr.fsr) $display("Not FULL!, %h, %h", refRxReg[0][1], refFsr[1]);
//                     else $display("FULL!, %h, %h", refRxReg[0][1], refFsr[1]);
//                 end
//                 // else begin 
//                 //     if (refRxReg[1]== ~rx_tr.rd_en)
//                 //         $display(
//                 //             "rd_en PASS!, %h, %h",
//                 //             refFndReg[1][3:0],
//                 //             ~fnd_tr.fndCom[3:0]
